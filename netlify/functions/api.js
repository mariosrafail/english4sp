...
function normalizePath(event) {
  ...
  if (p.startsWith(apiPrefix + "/")) return p.slice(apiPrefix.length);
  ...
  try {
    ...
    if (path.startsWith("/admin/")) {
      ...
      if (method === "POST" && path === "/admin/create-session") {
        ...
        const host = (event.headers && (event.headers["x-forwarded-host"] || event.headers["X-Forwarded-Host"])) || (event.headers && event.headers.host) || "localhost";
