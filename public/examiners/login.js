...
async function refreshMe(){
  const r = await fetch("/api/examiner/me", { credentials: "same-origin" });
...
async function onLogin(){
  ...
  try{
    ...
    const r = await fetch("/api/examiner/login", {
      method:"POST",
    ...
    setTimeout(()=>{ location.href = "/examiners/candidates.html"; }, 300);
  }catch(e){
...
async function onLogout(){
  ...
  try{
    await fetch("/api/examiner/logout", { method:"POST", credentials:"same-origin" });
    show("Logged out.", "ok");
