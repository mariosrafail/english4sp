...
function getSecret() {
  ...
  if (s) return s;
  // Fallback for local dev. For production, ADMIN_SESSION_SECRET MUST be set.
  if (!global.__ADMIN_SESSION_SECRET_FALLBACK__) {
    global.__ADMIN_SESSION_SECRET_FALLBACK__ = crypto.randomBytes(32).toString("hex");
  }
  return global.__ADMIN_SESSION_SECRET_FALLBACK__;
}
...
function createAdminToken(username, ttlMs = 8 * 60 * 60 * 1000) {
  const now = Date.now();
...
function setAuthCookie(req, res, token) {
  ...
  const parts = [
    `admin_auth=${encodeURIComponent(String(token || ""))}`,
    "Path=/",
    "HttpOnly",
    "SameSite=Strict",
    secure ? "Secure" : null,
...
// Very small in-memory login rate limit (per IP). Good enough to prevent brute force.
const _rl = new Map();
